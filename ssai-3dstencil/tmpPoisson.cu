...
////////////////////////////////////
#if 1
#define    _F001         ((T)-0.083300)
#define    _F010         ((T)0.083300)
#define    _F011         ((T)-0.166000)
#define    _F012         ((T)0.083300)
#define    _F021         ((T)-0.083300)
#define    _F100         ((T)0.083300)
#define    _F101         ((T)-0.166000)
#define    _F102         ((T)0.083300)
#define    _F110         ((T)-0.166000)
#define    _F111         ((T)2.666000)
#define    _F112         ((T)-0.166000)
#define    _F120         ((T)0.083300)
#define    _F121         ((T)-0.166000)
#define    _F122         ((T)0.083300)
#define    _F201         ((T)-0.083300)
#define    _F210         ((T)0.083300)
#define    _F211         ((T)-0.166000)
#define    _F212         ((T)0.083300)
#define    _F221         ((T)-0.083300)
#else
...
__global__ void kernel3d_poisson(const T * __restrict__ src, T* dst,
...
#pragma unroll
		...
		for (int ite = 0; ite < ITERATIVE_COUNT; ite++) {
			...
			if (warpId >= HALF_FILTER_DEPTH*ite && warpId < WARP_COUNT - HALF_FILTER_DEPTH*2 - HALF_FILTER_DEPTH*ite) {
				#pragma unroll
				for (int i = 0; i < CURRENT_PROCESS_DATA; i++) {
					...
					}
					{
						...
						sum = MAD(data[i + 1], _F012, sum);
						//sum = MAD(data[i + 2], _F022, sum);
					}
					{
						//m = 1
						sum = __my_shfl_down(sum, 1);
						sum = MAD(data[i + 0], _F001, sum);
						sum = MAD(data[i + 1], _F011, sum);
						sum = MAD(data[i + 2], _F021, sum);
					}
			...
			if (warpId >= HALF_FILTER_DEPTH * 2+HALF_FILTER_DEPTH*ite && warpId < WARP_COUNT - HALF_FILTER_DEPTH*ite) {
#pragma unroll
				for (int i = 0; i < CURRENT_PROCESS_DATA; i++) {
					T sum = 0;
					{
						//m = 0
						//sum = MAD(data[i + 0], _F200, sum);
						sum = MAD(data[i + 1], _F210, sum);
						//sum = MAD(data[i + 2], _F220, sum);
					}
					{
						//m = 2
						sum = __my_shfl_up(sum, 2);
						//sum = MAD(data[i + 0], _F202, sum);
						sum = MAD(data[i + 1], _F212, sum);
						//sum = MAD(data[i + 2], _F222, sum);
					}
					{
						//m = 1
						sum = __my_shfl_down(sum, 1);
						sum = MAD(data[i + 0], _F201, sum);
						sum = MAD(data[i + 1], _F211, sum);
						sum = MAD(data[i + 2], _F221, sum);
					}
					if (laneId >= HALF_FILTER_WIDTH*(ite + 1) && laneId < WARP_SIZE - HALF_FILTER_WIDTH*(ite + 1)) {
						sMem[warpId - 1 - HALF_FILTER_DEPTH][i + HALF_FILTER_HEIGHT - HALF_FILTER_HEIGHT][laneId - HALF_FILTER_WIDTH] += sum;
					}
			...
			if (warpId >= HALF_FILTER_DEPTH*(ite + 1) && warpId < WARP_COUNT - HALF_FILTER_DEPTH*(ite + 1)) {
#pragma unroll
				for (int i = 0; i < CURRENT_PROCESS_DATA; i++) {
					T sum = 0;
					{
						//m = 0
						sum = MAD(data[i + 0], _F100, sum);
						sum = MAD(data[i + 1], _F110, sum);
						sum = MAD(data[i + 2], _F120, sum);
					}
					{
						//m = 2
						sum = __my_shfl_up(sum, 2);
						sum = MAD(data[i + 0], _F102, sum);
						sum = MAD(data[i + 1], _F112, sum);
						sum = MAD(data[i + 2], _F122, sum);
					}
					{
						//m = 1
						sum = __my_shfl_down(sum, 1);
						sum = MAD(data[i + 0], _F101, sum);
						sum = MAD(data[i + 1], _F111, sum);
						sum = MAD(data[i + 2], _F121, sum);
					}
					if (laneId >= HALF_FILTER_WIDTH*(ite + 1) && laneId < WARP_SIZE - HALF_FILTER_WIDTH*(ite + 1)) {
...
static void host_code(T *h_in, T *h_out, int N) {
...

	{
...

		{
			...
			kernel3d_poisson<T, _FILTER_SIZE, BLOCK_PROCESS_DATA_COUNT_Y, BLOCK_PROCESS_DATA_COUNT_Z, WARP_COUNT, ITERATIVE_COUNT> << <grid_size, block_size >> >
			...
			printf("dtype=%s, all_time=%fms, one_itr_time=%fms, flop = %d, N=%d,Iterative=%d, CacheCount=%d, WarpCount=%d,  gups=%.2fGUPS, gflop=%.2fGFLOP/s\n", 
				sizeof(T)==8?"double":"float", ftime, ftime/ ITERATIVE_COUNT, flop, N, ITERATIVE_COUNT, CACHE_DATA_COUNT, WARP_COUNT, gups, flops);
		}
	}
...
void poisson_gold(const T* l_in, T* l_out, int N) {
		...
		for (int j = 1; j < N - 1; j++) {
			for (int i = 1; i < N - 1; i++) {
				out[k][j][i] = 2.666*in[k][j][i]
					- 0.166*(in[k - 1][j][i] + in[k + 1][j][i] + in[k][j - 1][i] + in[k][j + 1][i] + in[k][j][i + 1] + in[k][j][i - 1])
					+ 0.0833*(-in[k - 1][j - 1][i] - in[k + 1][j - 1][i] - in[k - 1][j + 1][i] - in[k + 1][j + 1][i]
						+ in[k - 1][j][i - 1] + in[k + 1][j][i - 1] + in[k][j - 1][i - 1] + in[k][j + 1][i - 1]
						+ in[k - 1][j][i + 1] + in[k + 1][j][i + 1] + in[k][j - 1][i + 1] + in[k][j + 1][i + 1]);
			}
		}
	}
...
static int _poisson(int argc, char** argv) {
	...
	for (int i = 0; i < ITERATIVE_COUNT; i++) {
		poisson_gold<T, N>((T*)input, (T*)output_gold, N);
...
int stencil_poisson(int argc, char** argv) {
	...
	if (IsTeslaV100()) {
		...
		//	_poisson<double, ITERATIVE_COUNT, N, B, 16>(argc, argv);
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 20>(argc, argv);
		//}
		{
			//dtype=double, all_time=4.887616ms, one_itr_time=2.443808ms, flop = 21, N=512,Iterative=2, CacheCount=16, WarpCount=16,  gups=54.92GUPS, gflop=1153.35GFLOP/s
			//dtype = float, all_time = 2.196800ms, one_itr_time = 1.098400ms, flop = 21, N = 512, Iterative = 2, CacheCount = 16, WarpCount = 16, gups = 122.19GUPS, gflop = 2566.07GFLOP / s
			const int ITERATIVE_COUNT = 2;
			if (bDouble) _poisson<double, ITERATIVE_COUNT, N, B, 16>(argc, argv);
			if (!bDouble) _poisson<float, ITERATIVE_COUNT, N, 512, 16>(argc, argv);
		}
		//{
		//	//dtype=double, all_time=4.887616ms, one_itr_time=2.443808ms, flop = 21, N=512,Iterative=2, CacheCount=16, WarpCount=16,  gups=54.92GUPS, gflop=1153.35GFLOP/s
		//	//dtype = float, all_time = 2.196800ms, one_itr_time = 1.098400ms, flop = 21, N = 512, Iterative = 2, CacheCount = 16, WarpCount = 16, gups = 122.19GUPS, gflop = 2566.07GFLOP / s
		//	const int ITERATIVE_COUNT = 3;
		//	_poisson<double, ITERATIVE_COUNT, N, B, 16>(argc, argv);
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 16>(argc, argv);
		//}
		//{
		//	const int ITERATIVE_COUNT = 3;
		//	_poisson<double, ITERATIVE_COUNT, N, B, 16>(argc, argv);
		...
		{
			const int ITERATIVE_COUNT = 3;
			if (bDouble) _poisson<double, ITERATIVE_COUNT, N, B, C>(argc, argv);
			if (!bDouble) _poisson<float, ITERATIVE_COUNT, N, 512, 16>(argc, argv);
		}
	} else {
		...
		//{
		//	const int ITERATIVE_COUNT = 1;
		//	_poisson<double, ITERATIVE_COUNT, N, 512, 12>(argc, argv);
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 24>(argc, argv);
		//}
		//{
		//	const int ITERATIVE_COUNT = 1;
		//	_poisson<double, ITERATIVE_COUNT, N, 512, 14>(argc, argv);
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 20>(argc, argv);
		//}
		//{
		//	const int ITERATIVE_COUNT = 1;
		//	_poisson<double, ITERATIVE_COUNT, N, 512, 16>(argc, argv);
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 22>(argc, argv);
		//}
		//{
		//	const int ITERATIVE_COUNT = 2;
		//	_poisson<double, ITERATIVE_COUNT, N, 512, 12>(argc, argv);
		...
		//	_poisson<float, ITERATIVE_COUNT, N, 512, 22>(argc, argv);
		//}
		//dtype=double, all_time=7.698048ms, one_itr_time=3.849024ms, N=512,Iterative=2, CacheCount=14, WarpCount=16,  gups=34.87GUPS, gflop=871.76GFLOP/s
		//dtype = float, all_time = 3.205664ms, one_itr_time = 1.602832ms, N = 512, Iterative = 2, CacheCount = 20, WarpCount = 16, gups = 83.74GUPS, gflop = 2093.45GFLOP / s
		{
			const int ITERATIVE_COUNT = 2;
			if (bDouble) _poisson<double, ITERATIVE_COUNT, N, 512, 16>(argc, argv);
			if (!bDouble) _poisson<float, ITERATIVE_COUNT, N, 512, 20>(argc, argv);
		}
		{
			const int ITERATIVE_COUNT = 3;
			if (bDouble) _poisson<double, ITERATIVE_COUNT, N, B, C>(argc, argv);
		...
		}
		{
			const int ITERATIVE_COUNT = 3;
			if (bDouble) _poisson<double, ITERATIVE_COUNT, N, B, C>(argc, argv);
			if (!bDouble) _poisson<float, ITERATIVE_COUNT, N, 512, 18>(argc, argv);
		}
	} 

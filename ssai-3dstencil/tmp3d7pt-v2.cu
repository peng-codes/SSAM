...
#pragma unroll
			for (int i = 0; i < CURRENT_PROCESS_DATA; i++) {
				...
				if (laneId >= HALF_FILTER_WIDTH*(ite + 1) && laneId < WARP_SIZE - HALF_FILTER_WIDTH*(ite + 1)) {
					sum += sMem[warpId - 1][i + HALF_FILTER_HEIGHT - HALF_FILTER_HEIGHT][laneId - HALF_FILTER_WIDTH] * _F3d7PT_011;
					sum += sMem[warpId + 1][i + HALF_FILTER_HEIGHT - HALF_FILTER_HEIGHT][laneId - HALF_FILTER_WIDTH] * _F3d7PT_211;
					data[i] = sum;
				}
			}
			__syncthreads();
...
int stencil_3d7pt_v2(int argc, char** argv) {
	using namespace v2;

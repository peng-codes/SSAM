...
namespace stencil2d_121pt_v4 {
	...
	__global__ void j2d121pt(const T* __restrict__ src, T* dst, int width, int height)
	{
		...
		int _index = width*_y + _x;

		{
			index = width*tidy + tidx;
...
#pragma unroll
			for (int s = 0; s < FILTER_HEIGHT - 1; s++, _tidy ++) {
				//int _tidy = tidy + s;
				data[s] = src[index];
...
#pragma unroll
		for (int i = 0; i < PROCESS_DATA_COUNT; i++) {
			data[FILTER_HEIGHT - 1 + i] = src[index];
			if (_tidy >= 0 && _tidy < height - 1) {				
				index += width;
			}
			_tidy++;
			...
			if (laneId < WARP_SIZE - FILTER_WIDTH + 1 && _x < width) {
				if (_y < height) {
					dst[_index] = sum;
					_index += width;
				}
...
int stencil_121pt_v4(int argc, char** argv) {
	DISPLAY_FUNCTION("");
	...
	int size = 8192; if (argc > 1) size = atoi(argv[1]);
	{
		const int P = 16;
		const int B = 128;
		stencil2d_121pt_v4::Test<double, P, B>(size, size);
	}
	return 0;
}

...
int stencil_121pt_double(int argc, char** argv) {
	DISPLAY_FUNCTION("");
	...
	int size = 8192; if (argc > 1) size = atoi(argv[1]);
	{
		const int P = 4;
		const int B = 128;
		stencil2d_121pt::Test<double, P, B>(size, size);
	}
	return 0;
}

int stencil_121pt_float(int argc, char** argv) {
	DISPLAY_FUNCTION("");
	printf("datatype=double\n");
	//cudaDeviceSetCacheConfig(cudaFuncCachePreferL1);
	int size = 8192; if (argc > 1) size = atoi(argv[1]);
	{
		const int P = 4;
		const int B = 128;
		stencil2d_121pt::Test<float, P, B>(size, size);
	}
	return 0;
}
